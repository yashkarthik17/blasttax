...
function AccordionSection({ id, icon, iconBg, iconColor, title, subtitle, defaultOpen = false, children }: {
  ...
  return (
    <div style={{ background: 'white', border: '1px solid #E8E8F0', borderRadius: '16px', overflow: 'hidden', marginBottom: '10px', transition: 'all 0.3s ease' }}>
      <button type="button" onClick={() => setOpen(!open)} style={{
        ...
        <div style={{ flex: 1, textAlign: 'left' }}>
          <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#1A1A2E' }}>{title}</div>
          {subtitle && <div style={{ fontSize: '0.7rem', color: '#8585A0' }}>{subtitle}</div>}
        </div>
        <i className="fas fa-chevron-down" style={{ fontSize: '11px', color: '#B0B0C8', transition: 'transform 0.3s ease', transform: open ? 'rotate(180deg)' : 'none' }} />
      </button>
...
export default function Form433BPage() {
  ...
  const inputStyle: React.CSSProperties = {
    width: '100%', padding: '12px 16px', background: '#FAFAFF', border: '1.5px solid #E8E8F0',
    borderRadius: '12px', fontFamily: 'inherit', fontSize: '0.85rem', fontWeight: 600, color: '#1A1A2E',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', paddingTop: '8px', paddingBottom: '40px' }}>
      ...
      <div>
        ...
        <div style={{ fontSize: '0.82rem', color: '#8585A0', marginTop: '6px', lineHeight: 1.5 }}>Required for business tax debt resolutions</div>
      </div>

      {/* Business Info Card */}
      <div style={{ background: 'white', borderRadius: '16px', padding: '16px', border: '1px solid #E8E8F0' }}>
        <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#B0B0C8', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '12px' }}>Business Details</div>

        <div style={{ marginBottom: '10px' }}>
          <div style={{ fontSize: '0.72rem', fontWeight: 600, color: '#5C5C7A', marginBottom: '6px' }}>Business Name</div>
          <input type="text" defaultValue="Doe's Consulting LLC" readOnly style={inputStyle} />
        </div>

        <div style={{ marginBottom: '10px' }}>
          <div style={{ fontSize: '0.72rem', fontWeight: 600, color: '#5C5C7A', marginBottom: '6px' }}>EIN</div>
        ...
        <div>
          <div style={{ fontSize: '0.72rem', fontWeight: 600, color: '#5C5C7A', marginBottom: '6px' }}>Business Type</div>
      ...
      <div>
        ...
        <AccordionSection id="acc-bank" icon="fa-university" iconBg="#EBF0FF" iconColor="#1A1A2E" title="Business Bank Accounts" defaultOpen>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', background: '#FAFAFF', borderRadius: '10px', marginBottom: '8px' }}>
            <div>
              <div style={{ fontSize: '0.82rem', fontWeight: 600, color: '#1A1A2E' }}>Chase Business Checking</div>
              <div style={{ fontSize: '0.7rem', color: '#8585A0' }}>****6789</div>
            </div>
            <span style={{ fontSize: '0.88rem', fontWeight: 700, color: '#1A1A2E' }}>$12,340</span>
          </div>
          <button style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '8px 14px', background: '#EBF0FF', border: 'none', borderRadius: '9999px', fontFamily: 'inherit', fontSize: '0.72rem', fontWeight: 700, color: '#1A1A2E', cursor: 'pointer' }}>
        ...
        <AccordionSection id="acc-receivable" icon="fa-file-invoice-dollar" iconBg="#E6F9EE" iconColor="#00A651" title="Accounts Receivable" subtitle="Total: $8,500">
          <div style={{ marginTop: '8px' }}>
            <div style={{ fontSize: '0.72rem', fontWeight: 600, color: '#5C5C7A', marginBottom: '6px' }}>Total Outstanding</div>
        ...
        <AccordionSection id="acc-assets" icon="fa-box" iconBg="#F5F0FF" iconColor="#7C3AED" title="Business Assets" subtitle="Equipment, inventory, vehicles">
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', paddingTop: '4px' }}>
            {[{ label: 'Equipment', amount: '$15,000' }, { label: 'Inventory', amount: '$6,200' }, { label: 'Vehicles', amount: '$8,500' }].map((item, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', background: '#FAFAFF', borderRadius: '10px' }}>
                <span style={{ fontSize: '0.82rem', fontWeight: 600, color: '#1A1A2E' }}>{item.label}</span>
                <span style={{ fontSize: '0.82rem', fontWeight: 700, color: '#1A1A2E' }}>{item.amount}</span>
              </div>
            ))}
            <button style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '8px 14px', background: '#EBF0FF', border: 'none', borderRadius: '9999px', fontFamily: 'inherit', fontSize: '0.72rem', fontWeight: 700, color: '#1A1A2E', cursor: 'pointer' }}>
        ...
        <AccordionSection id="acc-income" icon="fa-chart-line" iconBg="#FFFBEB" iconColor="#D97706" title="Business Income" subtitle="Monthly gross receipts">
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', paddingTop: '4px' }}>
            <div>
              <div style={{ fontSize: '0.72rem', fontWeight: 600, color: '#5C5C7A', marginBottom: '6px' }}>Monthly Gross Receipts</div>
              <input type="text" value={grossReceipts} onChange={(e) => setGrossReceipts(e.target.value)} placeholder="$0" style={inputStyle} />
            </div>
            <div>
              <div style={{ fontSize: '0.72rem', fontWeight: 600, color: '#5C5C7A', marginBottom: '6px' }}>Cost of Goods Sold</div>
      ...
      <div style={{ background: '#EBF0FF', borderRadius: '16px', padding: '16px', border: '1px solid rgba(0,61,165,0.12)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
          ...
          <span style={{ fontSize: '1.1rem', fontWeight: 900, color: '#1A1A2E' }}>$2,550/mo</span>
        </div>
