...
const BENEFITS = [
  ...
  { title: 'Filing Extensions', desc: 'Automatic 180-day extension after leaving combat zone', iconBg: 'bg-[#EBF0FF]', iconColor: 'text-[#1A1A2E]' },
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFF]">
      ...
      <div className="flex items-center justify-between px-5 py-3.5">
        ...
        <div className="flex-1 text-center text-[0.95rem] font-extrabold text-[#1A1A2E]">Military Tax Relief</div>
      ...
      <div className="flex flex-col gap-3.5 px-5 pb-8">
        ...
        <div className="flex items-center gap-2 px-1">
          <div className="text-[0.7rem] font-bold text-[#B0B0C8] uppercase tracking-wider">SCRA Benefits</div>
        ...
        {BENEFITS.map((benefit) => (
          <div
            key={benefit.title}
            className="flex items-start gap-3 rounded-[14px] border border-[#D5D5E0] bg-white p-3.5 transition-all hover:shadow-[0_1px_2px_rgba(0,0,0,0.03)] hover:-translate-y-px"
          >
            ...
            <div className="flex-1">
              <div className="text-[0.82rem] font-bold text-[#1A1A2E]">{benefit.title}</div>
              <div className="text-[0.72rem] text-[#8585A0] mt-0.5 leading-relaxed">{benefit.desc}</div>
            </div>
          </div>
        ))}
        ...
        <div className="rounded-[18px] border border-[#D5D5E0] bg-white p-[18px] shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
          <div className="text-[0.7rem] font-bold text-[#B0B0C8] uppercase tracking-wider mb-3">Qualifying Service</div>
          {QUALIFYING_SERVICE.map((item) => (
            <div key={item} className="flex items-center gap-2 py-[7px]">
              <div className="flex h-5 w-5 items-center justify-center rounded-md bg-[#1A1A2E] shrink-0">
        ...
        <div className="rounded-[18px] border border-[#D5D5E0] bg-white p-[18px] shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
          <div className="flex items-center gap-2 mb-3.5">
            ...
            <span className="text-[0.85rem] font-bold text-[#1A1A2E]">How to Claim</span>
          </div>

          {HOW_TO_CLAIM.map((step, index) => (
            <div key={step.title} className={`flex items-start gap-3 py-3 ${index < HOW_TO_CLAIM.length - 1 ? 'border-b border-[#F0F0F5]' : ''}`}>
