  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <div className="flex items-center justify-between px-5 py-3.5">
        ...
        <div className="flex-1 text-center text-[0.95rem] font-extrabold text-[#0A1628]">Statutory Exception</div>
      ...
      <div className="flex flex-col gap-3.5 px-5 pb-8">
        ...
        <div className="rounded-[14px] bg-[#F8FAFC] border border-[#E2E8F0] p-3.5 px-4">
          <p className="text-[0.78rem] text-[#64748B] leading-relaxed m-0">
        ...
        {SITUATIONS.map((situation, index) => (
          <button
            ...
            className={`w-full text-left rounded-2xl bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.03)] transition-all ${
              expandedCard === index ? 'border-[1.5px] border-[#0A1628]' : 'border border-[#E2E8F0]'
            }`}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-[0.85rem] font-bold text-[#0A1628]">{situation.title}</div>
                  <div className="text-[0.68rem] text-[#94A3B8] mt-px">{situation.code}</div>
                </div>
              </div>
              <svg
                className={`h-3 w-3 text-[#CBD5E1] transition-transform ${expandedCard === index ? 'rotate-180' : ''}`}
            ...
            {expandedCard === index && (
              <div className="mt-3.5 pt-3 border-t border-[#F1F5F9]">
                <p className="text-[0.75rem] text-[#64748B] leading-relaxed mb-2.5">{situation.description}</p>
                <div className="flex flex-col gap-1.5">
                  {situation.checks.map((check) => (
                    <div key={check} className="flex items-center gap-1.5">
                      ...
                      <span className="text-[0.72rem] text-[#0A1628] font-medium">{check}</span>
                    </div>
                  ))}
                  <div className="flex items-center gap-1.5">
                    <svg className="h-3 w-3 text-[#0A1628] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                    <span className="text-[0.72rem] text-[#64748B] font-medium">{situation.claim}</span>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="rounded-[18px] border border-[#E2E8F0] bg-white p-[18px] shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
          <div className="flex items-center gap-2 mb-3.5">
            ...
            <div className="text-[0.85rem] font-bold text-[#0A1628]">Check If You Qualify</div>
          </div>

          <p className="text-[0.75rem] text-[#64748B] leading-relaxed mb-3.5">
              ...
              <button
                ...
                className={`flex items-center gap-2.5 rounded-xl px-3 py-2.5 border-[1.5px] transition-all text-left ${
                  selectedQualify === index
                    ? 'border-[#0A1628] bg-[#EBF0FF]'
                    : 'border-[#E2E8F0] bg-[#F8FAFC] hover:border-[#0A1628] hover:bg-[#EBF0FF]'
                }`}
              >
                <div
                  className={`flex h-5 w-5 items-center justify-center rounded-full border-2 shrink-0 transition-all ${
                    selectedQualify === index ? 'border-[#0A1628] bg-[#0A1628]' : 'border-[#E2E8F0]'
                  }`}
                >
                  {selectedQualify === index && (
                ...
                <span className="text-[0.78rem] font-semibold text-[#0A1628]">{option}</span>
              </button>
            ))}
          </div>
