...
const incomeTypes = [
  ...
  { id: 'other', label: 'Other Income', sub: 'Gambling, alimony, etc.', icon: '...', bgColor: 'bg-[#F8FAFC]' },
]
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F8FAFC]">
      ...
      <div className="flex items-center justify-between px-5 py-3.5">
        <Link href="/tax-filing/security" className="w-9 h-9 rounded-xl bg-[#F8FAFC] border border-[#F1F5F9] flex items-center justify-center">
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-24 space-y-4">
        {/* Title */}
        <div className="pt-1">
          <h1 className="text-[1.35rem] font-extrabold text-[#0A1628] tracking-tight mb-1.5">What income did you receive?</h1>
          <p className="text-[0.82rem] text-[#94A3B8] font-normal leading-relaxed">Select all that apply for tax year 2025</p>
        </div>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3.5 rounded-[14px] border-[1.5px] select-none transition-all hover:-translate-y-px active:scale-[0.98] text-left ${
                  isSelected
                    ? 'border-[#0A1628] bg-[#EFF4FF] shadow-[0_0_0_3px_rgba(10,22,40,0.08)]'
                    : 'border-[#E2E8F0] bg-white hover:border-[#0A1628]/20'
                }`}
              >
                ...
                <div className="flex-1">
                  <div className="text-[0.88rem] font-semibold text-[#0A1628]">{item.label}</div>
                ...
                <div className={`w-[22px] h-[22px] rounded-md border-2 flex items-center justify-center shrink-0 transition-all ${
                  isSelected ? 'border-[#0A1628] bg-[#0A1628]' : 'border-[#D5D5E0]'
                }`}>
                  {isSelected && (
        ...
        <div className="flex items-center gap-1.5 px-1">
          <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="#CBD5E1" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path strokeLinecap="round" d="M12 16v-4m0-4h.01"/></svg>
          <span className="text-[0.72rem] text-[#94A3B8] font-medium">At least one must be selected</span>
        </div>

        {/* Continue Button */}
        <div className="pt-1">
          <Link href="/tax-filing/w2" className="w-full flex items-center justify-center gap-2 bg-[#0A1628] text-white text-[0.95rem] font-bold py-4 px-7 rounded-full hover:-translate-y-px active:scale-[0.97] transition-all">
            Continue
