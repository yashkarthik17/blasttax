  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F8FAFC]">
      ...
      <div className="flex items-center justify-between px-5 py-3.5">
        <Link href="/dashboard" className="w-9 h-9 rounded-xl bg-[#F8FAFC] border border-[#F1F5F9] flex items-center justify-center">
        ...
        <span className="text-[0.95rem] font-bold text-[#0A1628]">Tax Filing</span>
        <button className="w-9 h-9 rounded-xl flex items-center justify-center">
          <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#0A1628" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path strokeLinecap="round" d="M12 16v-4m0-4h.01"/></svg>
        </button>
      </div>

      <div className="flex-1 overflow-y-auto px-5 pb-24 space-y-[18px]">
        {/* Hero Card */}
        <div className="bg-[#0A1628] rounded-[20px] p-6 relative overflow-hidden">
        ...
        {/* Tax Year Cards */}
        <div>
          <div className="text-[0.75rem] font-bold text-[#CBD5E1] uppercase tracking-wider mb-3 px-1">Filing Status by Year</div>
          <div className="flex flex-col gap-2.5">
            {yearData.map((item) => (
              <div key={item.year} className="bg-white rounded-[16px] px-[18px] py-4 border border-[#E2E8F0] flex items-center gap-3.5 hover:-translate-y-px transition-all cursor-pointer active:scale-[0.98]">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-[0.92rem] font-bold text-[#0A1628]">{item.year}</span>
                ...
                {item.action === 'view' && (
                  <div className="flex gap-1.5 items-center">
                    <a href="#" className="text-[0.72rem] font-semibold text-[#0A1628]">View</a>
                    <span className="text-[#E2E8F0]">|</span>
                    <a href="#" className="text-[0.72rem] font-semibold text-[#94A3B8]">Amend</a>
                  </div>
                )}
        ...
        {/* CTA */}
        <div className="pt-1">
          <Link href="/tax-filing/personal" className="w-full flex items-center justify-center gap-2 bg-[#0A1628] text-white text-[0.95rem] font-bold py-4 px-7 rounded-full hover:-translate-y-px active:scale-[0.97] transition-all">
            Start Filing 2025
