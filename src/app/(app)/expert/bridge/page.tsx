    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="mx-auto max-w-md">
        ...
        <div className="flex items-center gap-3 px-5 py-4">
          <Link href="/dashboard" className="flex h-10 w-10 items-center justify-center rounded-xl bg-white border border-[#F1F5F9]">
        ...
        <div className="px-5 pb-8 space-y-4">
          ...
          <div className="text-center py-4">
            <div className="w-16 h-16 rounded-2xl bg-[#EFF4FF] flex items-center justify-center mx-auto mb-4">
              <svg className="w-7 h-7 text-[#0A1628]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 5.25h.008v.008H12v-.008Z" />
              </svg>
            </div>
            <h2 className="text-[1.375rem] font-bold text-[#0A1628] mb-1.5">Get Expert Help</h2>
            <p className="text-[0.8125rem] text-[#6B7280] leading-relaxed">Connect with a licensed tax professional for personalized guidance on your case.</p>
          </div>

          {/* Select Case Section */}
          <p className="text-[0.6875rem] font-bold text-[#6B7280] uppercase tracking-wider">Select a case</p>
          ...
          {cases.map((c) => (
            <Link
              ...
              className={`flex items-center gap-3.5 p-4 bg-white border-[1.5px] rounded-2xl transition-colors ${
                selectedCase === c.id ? 'border-[#0A1628] bg-[#EFF4FF]' : 'border-[#F1F5F9] hover:border-[#0A1628]'
              }`}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <p className="text-[0.8125rem] font-semibold text-[#0A1628]">Case #{c.id}</p>
                  <span className={`px-2 py-0.5 rounded-full text-[0.625rem] font-bold ${c.statusBg} ${c.statusColor}`}>{c.status}</span>
                </div>
                <p className="text-xs text-[#6B7280]">{c.type} &mdash; {c.amount}</p>
              </div>
          ...
          <Link
            href="/expert/landing"
            className="flex items-center justify-center gap-2 p-5 bg-white border-[1.5px] border-dashed border-[#F1F5F9] rounded-2xl hover:border-[#0A1628] transition-colors"
          >
            <svg className="w-3.5 h-3.5 text-[#0A1628]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
            </svg>
            <p className="text-[0.8125rem] font-semibold text-[#0A1628]">Start without a case</p>
          </Link>

          {/* Info */}
          <div className="mt-5">
            <div className="flex gap-3 p-3.5 bg-[#F1F5F9] rounded-xl items-start">
              ...
              <p className="text-xs text-[#6B7280] leading-relaxed">Our experts are licensed Enrolled Agents, CPAs, and Tax Attorneys with 10+ years of IRS experience.</p>
            </div>
          </div>
        </div>
      </div>
