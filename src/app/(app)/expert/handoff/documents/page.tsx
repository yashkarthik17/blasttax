    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="mx-auto max-w-md">
        ...
        <div className="flex items-center gap-3 px-5 py-4">
          <Link href="/expert/handoff/intake" className="flex h-10 w-10 items-center justify-center rounded-xl bg-white border border-[#F1F5F9]">
            <svg className="w-4 h-4 text-[#0A1628]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
            </svg>
          </Link>
          <h1 className="flex-1 text-center text-[0.95rem] font-extrabold text-[#0A1628]">Document Package</h1>
          <div className="w-10" />
        </div>
        ...
        <div className="px-5 pb-10 space-y-[18px]">
          {/* Heading */}
          <div>
            <div className="text-[1.25rem] font-extrabold text-[#0A1628] tracking-tight leading-tight">Review Your Document Package</div>
            <div className="text-[0.82rem] text-[#94A3B8] mt-1.5 leading-relaxed">These documents will be sent to your tax professional</div>
          </div>

          {/* Document Sections */}
          {sections.map((section) => (
            <div key={section.label}>
              <div className="text-xs font-bold text-[#CBD5E1] uppercase tracking-wider mb-2.5 flex items-center gap-1">
                {section.icon}
                {section.label}
              </div>
              <div className="bg-white rounded-2xl px-4 py-1 border border-[#F1F5F9]">
                {section.docs.map((doc, i) => (
                  <div key={doc.name} className={`flex items-center gap-3 py-3 ${i < section.docs.length - 1 ? 'border-b border-[#F1F5F9]' : ''}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[0.7rem] text-[#94A3B8]">{doc.pages}</div>
                    </div>
                    {doc.hasEye && (
                      <svg className="w-[13px] h-[13px] text-[#CBD5E1] cursor-pointer mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="flex items-center justify-center gap-2 py-2">
            ...
            <span className="text-[0.82rem] font-bold text-[#0A1628]">7 documents</span>
            <span className="text-[0.72rem] text-[#94A3B8]">&bull;</span>
            <span className="text-[0.82rem] font-bold text-[#0A1628]">36 pages</span>
          </div>
          ...
          <div className="flex flex-col gap-3 pt-1">
            ...
            <button className="flex items-center justify-center gap-1.5 w-full py-3 border-2 border-[#E2E8F0] bg-white text-[#0A1628] rounded-full text-[0.82rem] font-semibold hover:border-[#0A1628] transition">
              <svg className="w-[11px] h-[11px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
