...
const codes: TranscriptCode[] = [
  { code: 'TC 150', label: 'Return Filed', date: 'Apr 15, 2022', type: 'status', amountColor: '#10B981', iconBg: '#ECFDF5', iconColor: '#10B981', codeBg: '#EFF4FF', codeColor: '#0A1628', badge: { label: 'Filed', bg: '#ECFDF5', color: '#10B981' }, detail: 'Your 2021 tax return was received and processed by the IRS. This establishes the CSED start date for this tax year.' },
  { code: 'TC 806', label: 'W-2 Withholding', date: 'Apr 15, 2022', type: 'payment', amount: '-$6,240', amountColor: '#10B981', iconBg: '#EFF4FF', iconColor: '#0A1628', codeBg: '#EFF4FF', codeColor: '#0A1628', detail: 'Credit applied from W-2 income tax withholding. This amount was withheld by your employer throughout the tax year.' },
    ...
    <div className="min-h-screen bg-[var(--background)] p-4 sm:p-8">
      <div className="mx-auto max-w-2xl space-y-3.5">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/cases"
            className="flex h-9 w-9 items-center justify-center rounded-xl border border-[#F3F4F6] bg-[#F8FAFC] text-[#64748B] transition hover:bg-[#F1F5F9]"
          >
            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
            </svg>
          </Link>
          <h1 className="flex-1 text-center text-[0.95rem] font-extrabold text-[#0A1628]">Transcript Codes</h1>
          <div className="w-9" />
        </div>
            ...
            <button
              ...
              className={`whitespace-nowrap rounded-full border px-3.5 py-[7px] text-[0.72rem] font-semibold transition ${
                filter === f.value
                  ...
                  : 'border-[#F3F4F6] bg-white text-[#64748B] hover:border-[#0A1628] hover:bg-[#EFF4FF] hover:text-[#0A1628]'
              }`}
            >
              {f.label}
            </button>
        ...
        {/* Code List */}
        <div className="overflow-hidden rounded-[18px] border border-[#F3F4F6] bg-white shadow-sm">
          {filtered.map((c, i) => (
            <div
              key={c.code}
              className={`cursor-pointer transition hover:bg-[#F8FAFC] ${i < filtered.length - 1 ? 'border-b border-[#F1F5F9]' : ''}`}
              ...
              <div className="flex items-center gap-3 px-4 py-3.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-0.5 flex items-center gap-1.5">
                    ...
                    <span className="text-[0.82rem] font-bold text-[#0A1628]">{c.label}</span>
                  </div>
                  <div className="text-[0.68rem] text-[#94A3B8]">{c.date}</div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <svg className={`h-2.5 w-2.5 text-[#CBD5E1] transition-transform ${openIndex === i ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                  </svg>
                </div>
              </div>
              <div className={`overflow-hidden transition-all ${openIndex === i ? 'max-h-32 pb-3.5 pt-2.5' : 'max-h-0'}`}>
                <div className="pl-[46px] pr-4 text-[0.72rem] leading-relaxed text-[#64748B]">{c.detail}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Summary */}
        <div className="flex items-center justify-center gap-4 rounded-[14px] border border-[#F3F4F6] bg-white px-4 py-3 shadow-sm">
          <div className="flex items-center gap-1.5">
            <div className="h-2 w-2 rounded-sm bg-[#EF4444]" />
            <span className="text-[0.7rem] font-semibold text-[#64748B]">6 assessments</span>
          </div>
          <div className="h-4 w-px bg-[#E2E8F0]" />
          <div className="flex items-center gap-1.5">
            <div className="h-2 w-2 rounded-sm bg-[#D97706]" />
            <span className="text-[0.7rem] font-semibold text-[#64748B]">2 penalties</span>
          </div>
          <div className="h-4 w-px bg-[#E2E8F0]" />
          <div className="flex items-center gap-1.5">
            <div className="h-2 w-2 rounded-sm bg-[#10B981]" />
            <span className="text-[0.7rem] font-semibold text-[#64748B]">1 credit</span>
          </div>
        </div>
      </div>
    </div>
