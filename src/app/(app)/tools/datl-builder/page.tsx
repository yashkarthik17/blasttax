    ...
    <div className="min-h-screen bg-[var(--background)] p-4 sm:p-8">
      <div className="mx-auto max-w-2xl space-y-4">
        ...
        <div>
          ...
          <p className="mt-1.5 text-xs text-[#8585A0] leading-relaxed">
        ...
        {/* Qualifying Reasons */}
        <div>
          <div className="mb-2 text-xs font-bold uppercase tracking-wider text-[#8585A0]">Qualifying Reasons</div>
              ...
              <div key={idx}>
                <button
                  ...
                  className={`flex w-full items-start gap-3 rounded-[14px] border-[1.5px] p-3.5 text-left transition ${
                    selected.has(idx)
                      ? 'border-[#1A1A2E] bg-[#EFF4FF] shadow-[0_0_0_3px_rgba(10,22,40,0.1)]'
                      : 'border-[#E8E8F0] bg-white hover:-translate-y-0.5 hover:shadow-sm'
                  }`}
                >
                  <div className={`mt-0.5 flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-md border-2 transition ${
                    selected.has(idx) ? 'border-[#1A1A2E] bg-[#1A1A2E]' : 'border-[#D5D5E0]'
                  }`}>
                    {selected.has(idx) && (
                  ...
                  <div className="flex-1">
                    ...
                    {selected.has(idx) && (
                      <textarea
                        ...
                        className="mt-2.5 w-full resize-y rounded-[10px] border-[1.5px] border-[#E8E8F0] bg-white px-3 py-2.5 text-[13px] text-[var(--foreground)] outline-none transition focus:border-[#1A1A2E] focus:shadow-[0_0_0_3px_rgba(10,22,40,0.08)] placeholder:text-[#B0B0C8]"
                        rows={2}
                      />
                    )}
                  </div>
        ...
        {/* Supporting Evidence */}
        <div>
          <div className="mb-2 text-xs font-bold uppercase tracking-wider text-[#8585A0]">Supporting Evidence</div>
          <div className="flex cursor-pointer flex-col items-center gap-1.5 rounded-xl border-2 border-dashed border-[#F0F0F5] p-4 text-center transition hover:border-[#1A1A2E] hover:bg-[#EFF4FF]">
            <svg className="h-5 w-5 text-[#5C5C7A]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
            </svg>
            <div className="text-xs font-semibold text-[#5C5C7A]">Upload documents</div>
            <div className="text-[11px] text-[#B0B0C8]">PDF, JPG, PNG</div>
          </div>
        </div>

        {/* Tax Years */}
        <div>
          <div className="mb-2 text-xs font-bold uppercase tracking-wider text-[#8585A0]">Tax Years Affected</div>
              ...
              <button
                ...
                className={`rounded-full border-[1.5px] px-4 py-2 text-[13px] font-semibold transition ${
                  activeYears.has(year)
                    ...
                    : 'border-[#E8E8F0] bg-white text-[#5C5C7A]'
                }`}
              >
                {year}
              </button>
        ...
        {/* Disputed Amounts */}
        <div>
          <div className="mb-2 text-xs font-bold uppercase tracking-wider text-[#8585A0]">Disputed Amount Per Year</div>
          <div className="flex gap-2.5">
            {Array.from(activeYears).sort().map((year) => (
              <div key={year} className="flex-1">
                <div className="mb-1 text-[11px] font-semibold text-[#8585A0]">{year}</div>
                <input
                  type="text"
                  placeholder="$0"
                  value={amounts[year] || ''}
                  onChange={(e) => setAmounts({ ...amounts, [year]: e.target.value })}
                  className="w-full rounded-[10px] border-[1.5px] border-[#E8E8F0] bg-[#FAFAFF] px-3 py-2.5 text-[14px] font-semibold text-[var(--foreground)] outline-none transition focus:border-[#1A1A2E] focus:bg-white focus:shadow-[0_0_0_3px_rgba(10,22,40,0.08)] placeholder:font-normal placeholder:text-[#B0B0C8]"
                />
              </div>
            ))}
          </div>
        ...
        {/* Total Bar */}
        <div className="flex items-center justify-between rounded-[14px] bg-[#1A1A2E] px-[18px] py-3.5">
          <div>
        ...
        {/* Continue */}
        <div className="py-3">
          <button className="flex w-full items-center justify-center gap-2 rounded-full bg-[#1A1A2E] px-7 py-4 text-[15px] font-bold text-white transition hover:opacity-90">
            Continue
