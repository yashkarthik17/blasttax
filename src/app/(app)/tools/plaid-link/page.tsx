...
const benefits = [
  ...
  { icon: 'headset', color: '#0A1628', bg: '#EFF4FF', title: 'Accurate asset reporting', desc: 'Real balances, no guesswork' },
      ...
      <div className="mx-auto max-w-2xl space-y-5">
        ...
        <div className="flex justify-center pt-2">
          <div className="relative flex h-[72px] w-[72px] items-center justify-center rounded-[22px] bg-[#EFF4FF]">
            <svg className="relative z-10 h-7 w-7 text-[#0A1628]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="text-center">
          <div className="text-[1.35rem] font-extrabold leading-tight tracking-tight text-[#0A1628]">Securely link your accounts</div>
          <div className="mt-2 text-[0.82rem] font-medium leading-relaxed text-[#94A3B8]">Automatically import financial data for accurate analysis</div>
        </div>

        {/* Benefits */}
        <div className="space-y-1.5">
          {benefits.map((b) => (
            <div key={b.title} className="flex items-center gap-3 rounded-xl p-2.5 transition hover:bg-[#F1F5F9]">
              ...
              <div>
                <div className="text-[0.85rem] font-bold text-[#0A1628]">{b.title}</div>
                <div className="text-[0.72rem] text-[#94A3B8]">{b.desc}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div>
          <div className="relative mb-4">
            <svg className="absolute left-3.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[#CBD5E1]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            <input
              type="text"
              placeholder="Search your bank..."
              className="w-full rounded-xl border-[1.5px] border-[#F3F4F6] bg-[#F8FAFC] py-3 pl-10 pr-4 text-[0.85rem] font-medium text-[#0A1628] outline-none transition focus:border-[#0A1628] focus:bg-white placeholder:text-[#CBD5E1]"
            />
          </div>

          <div className="mb-3 text-[0.7rem] font-bold uppercase tracking-wider text-[#CBD5E1]">Popular Banks</div>
          <div className="flex justify-between gap-2.5">
            {banks.map((bank) => (
              <button key={bank.name} className="flex flex-col items-center gap-1.5 transition hover:-translate-y-0.5 active:scale-95">
                ...
                <span className="text-[0.65rem] font-semibold text-[#64748B]">{bank.name}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Security Card */}
        <div className="flex items-start gap-3 rounded-2xl border border-[#F3F4F6] bg-white p-4">
          ...
          <div>
            <div className="mb-1.5 text-[0.8rem] font-bold text-[#0A1628]">Bank-grade security</div>
            <div className="flex flex-wrap gap-1.5">
              {['256-bit encryption', 'Read-only access', 'Revoke anytime'].map((item) => (
                <span key={item} className="inline-flex items-center gap-1 text-[0.68rem] font-semibold text-[#64748B]">
        ...
        <div className="space-y-3 pt-1">
          <button className="flex w-full items-center justify-center gap-2 rounded-full bg-[#0A1628] px-7 py-4 text-[0.95rem] font-bold text-white transition hover:opacity-90">
            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
            </svg>
            Connect Bank Account
          </button>
          <button className="flex w-full items-center justify-center rounded-full px-7 py-3 text-[0.82rem] font-semibold text-[#94A3B8] transition hover:text-[#64748B]">
