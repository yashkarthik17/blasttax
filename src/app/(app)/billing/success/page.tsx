    ...
    <div className="flex flex-col min-h-screen items-center justify-center px-6 relative">
      ...
      <div className="relative z-10 flex flex-col items-center gap-5 w-full">
        ...
        <div className="w-20 h-20 rounded-full bg-gradient-to-br from-green-500 to-green-700 flex items-center justify-center shadow-lg shadow-green-500/25">
          <svg className="h-8 w-8 text-[#1A1A2E]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
        ...
        <div className="text-center">
          <h1 className="text-2xl font-extrabold text-[#1A1A2E] mb-1">Payment Successful!</h1>
          <p className="text-sm text-[#5C5C7A] font-medium">Your Pro Plan is now active</p>
        </div>

        {/* Receipt Summary Card */}
        <div className="w-full bg-white border border-[#F0F0F5] rounded-2xl p-[18px]">
          <div className="flex items-center gap-2 mb-3.5">
            ...
            <span className="text-sm font-bold text-[#1A1A2E]">Receipt Summary</span>
          </div>
            ...
            {receiptItems.map((item, i) => (
              <div key={item.label}>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-[#5C5C7A]">{item.label}</span>
                  <span className="text-sm font-semibold text-[#1A1A2E]">{item.value}</span>
                </div>
                {i === 2 && <div className="h-px bg-[#F0F0F5] mt-2.5" />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 text-center">
          ...
          <span className="text-xs text-[#8585A0] font-medium">Receipt sent to jane@email.com</span>
        </div>
        ...
        <div className="w-full flex flex-col gap-2.5 mt-1">
          <Link
            href="/dashboard"
            className="w-full flex items-center justify-center py-4 bg-white text-[#1A1A2E] rounded-full text-[0.9375rem] font-bold hover:bg-[#D5D5E0] transition-colors"
          >
            Go to Dashboard
          </Link>
          <Link
            href="/billing"
            className="w-full flex items-center justify-center py-3.5 bg-white border border-[#F0F0F5] text-[#1A1A2E] rounded-full text-sm font-semibold hover:border-[#D5D5E0] transition-colors"
          >
            View Billing
          </Link>
        </div>
