  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex items-center justify-between px-5 pt-4 pb-3">
        ...
        <h1 className="text-lg font-bold text-[#1A1A2E]">IRS Payments</h1>
        <div className="w-9" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-6 space-y-5">
        {/* Active Obligations */}
        <div>
          <p className="text-[0.6875rem] font-bold text-[#8585A0] uppercase tracking-wide mb-3">Active Obligations</p>

          {obligations.map((ob) => (
            <div key={ob.name} className={`bg-white border border-[#F0F0F5] rounded-[14px] p-3.5 mb-2.5 ${ob.paid ? 'opacity-75' : ''}`}>
              <div className="flex items-start justify-between mb-2.5">
                <div>
                  <p className="text-sm font-semibold text-[#1A1A2E]">{ob.name}</p>
                  <p className="text-xs text-[#8585A0] mt-0.5">{ob.detail}</p>
                </div>
              ...
              {ob.showPay && (
                <div className="flex justify-end">
                  <button className="px-5 py-2 bg-white text-[#1A1A2E] rounded-full text-xs font-bold hover:bg-[#D5D5E0] transition-colors">
                    Pay Now
                  </button>
                </div>
              )}
        ...
        {/* Payment Methods */}
        <div>
          <p className="text-[0.6875rem] font-bold text-[#8585A0] uppercase tracking-wide mb-3">Payment Methods</p>

          <div className="space-y-0">
            {paymentMethods.map((m, i) => (
              <div key={m.name} className={`flex items-center gap-3.5 py-3.5 ${i < paymentMethods.length - 1 ? 'border-b border-[#F0F0F5]' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[0.6875rem] text-[#8585A0] mt-0.5">{m.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Recent Payments */}
        <div>
          <p className="text-[0.6875rem] font-bold text-[#8585A0] uppercase tracking-wide mb-3">Recent Payments</p>

          {recentPayments.map((p, i) => (
            <div key={p.name} className={`flex items-start justify-between py-3 ${i < recentPayments.length - 1 ? 'border-b border-[#F0F0F5]' : ''}`}>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-[#1A1A2E]">{p.name}</p>
                <p className="text-[0.6875rem] text-[#8585A0] mt-0.5">{p.detail}</p>
              </div>
              <p className="text-sm font-bold text-[#1A1A2E] flex-shrink-0 ml-3">{p.amount}</p>
            </div>
          ))}
        </div>
      </div>
