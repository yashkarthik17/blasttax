    ...
    <div style={{ minHeight: '100vh', background: '#FAFAFF' }}>
      <div style={{ maxWidth: '28rem', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px 12px' }}>
          ...
          <span style={{ flex: 1, fontSize: '0.95rem', fontWeight: 800, color: '#1A1A2E', textAlign: 'center' }}>Plans</span>
          <div style={{ width: 36 }} />
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18, padding: '0 20px', paddingBottom: 20 }}>
          ...
          <div style={{ display: 'flex', background: '#F0F0F5', borderRadius: 12, padding: 4 }}>
            <button
              ...
              style={{ flex: 1, padding: '10px 16px', border: 'none', background: billing === 'monthly' ? 'white' : 'transparent', fontFamily: 'inherit', fontSize: '0.78rem', fontWeight: 600, color: billing === 'monthly' ? '#1A1A2E' : '#8585A0', cursor: 'pointer', borderRadius: 10, boxShadow: billing === 'monthly' ? '0 1px 2px rgba(0,0,0,0.03)' : 'none' }}
            >
              Monthly
            </button>
            <button
              onClick={() => setBilling('annual')}
              style={{ flex: 1, padding: '10px 16px', border: 'none', background: billing === 'annual' ? 'white' : 'transparent', fontFamily: 'inherit', fontSize: '0.78rem', fontWeight: 600, color: billing === 'annual' ? '#1A1A2E' : '#8585A0', cursor: 'pointer', borderRadius: 10, boxShadow: billing === 'annual' ? '0 1px 2px rgba(0,0,0,0.03)' : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}
            >
              Annual
          ...
          {/* Starter Plan */}
          <div style={{ background: 'white', border: '1.5px solid #D5D5E0', borderRadius: 20, padding: 22, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ background: '#FFFFFF', border: '2px solid #1A1A2E', borderRadius: 20, padding: 22, position: 'relative', overflow: 'hidden', boxShadow: '0 1px 2px rgba(0,0,0,0.03)' }}>
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: '#1A1A2E' }} />
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
              <div style={{ fontSize: '0.68rem', fontWeight: 600, color: '#1A1A2E', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Pro</div>
              <span style={{ padding: '4px 10px', background: '#1A1A2E', borderRadius: 9999, fontSize: '0.6rem', fontWeight: 700, color: 'white' }}>RECOMMENDED</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 3, marginBottom: 14 }}>
              <span style={{ fontSize: '1.6rem', fontWeight: 900, color: '#1A1A2E' }}>{proPrice}</span>
              <span style={{ fontSize: '0.82rem', color: '#8585A0', fontWeight: 500 }}>/mo</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {proFeatures.map((f, i) => (
                <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0' }}>
                  <i className="fas fa-check" style={{ fontSize: 11, color: i === 0 ? '#00A651' : '#1A1A2E' }} />
                  <span style={{ fontSize: '0.78rem', color: i === 0 ? '#5C5C7A' : '#1A1A2E', fontWeight: i === 0 ? 500 : 600 }}>{f}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 14, padding: '8px 12px', background: 'rgba(10,22,40,0.06)', borderRadius: 10 }}>
              <i className="fas fa-circle-check" style={{ fontSize: 12, color: '#1A1A2E' }} />
              <span style={{ fontSize: '0.72rem', fontWeight: 600, color: '#1A1A2E' }}>Your current plan</span>
            </div>
          </div>

          {/* Enterprise Plan */}
          <div style={{ background: '#1A1A2E', border: '1.5px solid transparent', borderRadius: 20, padding: 22, position: 'relative', overflow: 'hidden' }}>
          ...
          <div>
            <div
              ...
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', background: 'white', border: '1px solid #D5D5E0', borderRadius: 14, cursor: 'pointer' }}
            >
              <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#1A1A2E' }}>Feature Comparison</span>
              <i className="fas fa-chevron-down" style={{ fontSize: 11, color: '#8585A0', transition: 'transform 0.3s ease', transform: showComparison ? 'rotate(180deg)' : 'rotate(0deg)' }} />
            </div>
            {showComparison && (
              <div style={{ background: 'white', border: '1px solid #D5D5E0', borderTop: 'none', borderRadius: '0 0 14px 14px', padding: '8px 16px', marginTop: -4 }}>
                {/* Header */}
                <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr', alignItems: 'center', padding: '10px 0', borderBottom: '2px solid #D5D5E0', fontSize: '0.72rem' }}>
                  <div style={{ fontWeight: 600, color: '#B0B0C8', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Feature</div>
                  <div style={{ textAlign: 'center', fontWeight: 600, color: '#B0B0C8', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Starter</div>
                  <div style={{ textAlign: 'center', fontWeight: 700, color: '#1A1A2E', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Pro</div>
                  <div style={{ textAlign: 'center', fontWeight: 600, color: '#B0B0C8', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Enter.</div>
                </div>
                {comparisonRows.map((row, i) => (
                  <div key={row.label} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr', alignItems: 'center', padding: '10px 0', borderBottom: i < comparisonRows.length - 1 ? '1px solid #F0F0F5' : 'none', fontSize: '0.72rem' }}>
                    <div style={{ fontWeight: 600, color: '#5C5C7A' }}>{row.label}</div>
                    {[row.starter, row.pro, row.enterprise].map((val, j) => (
                      <div key={j} style={{ textAlign: 'center', fontWeight: 600, color: '#1A1A2E' }}>
                        {typeof val === 'boolean' ? (
                          val ? (
                            <i className="fas fa-check" style={{ fontSize: 11, color: '#00A651' }} />
                          ) : (
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 16px', background: '#FAFAFF', borderRadius: 9999 }}>
