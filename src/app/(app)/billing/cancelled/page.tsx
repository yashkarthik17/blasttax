...
export default function CancelConfirmedPage() {
  return (
    <div className="flex flex-col min-h-screen items-center justify-center px-7 text-center">
      ...
      {/* Heading */}
      <h1 className="text-[1.35rem] font-extrabold text-white leading-tight mb-2">Subscription Cancelled</h1>
      <p className="text-sm text-zinc-400 mb-6">We&apos;re sorry to see you go.</p>

      {/* Info Card */}
      <div className="w-full bg-blue-500/10 border border-blue-500/30 rounded-[14px] p-4 flex items-center gap-3 mb-6">
        <div className="w-9 h-9 rounded-[10px] bg-zinc-900 flex items-center justify-center flex-shrink-0">
        ...
        <div className="text-left">
          <div className="text-sm font-semibold text-blue-300 leading-snug">Your Pro access continues until April 15, 2026</div>
        </div>
      </div>

      {/* What Happens Next */}
      <div className="w-full text-left mb-6">
        <div className="text-xs font-bold text-zinc-600 uppercase tracking-wider mb-3.5">What happens next</div>
        <div className="flex flex-col gap-3">
          {nextSteps.map((step) => (
            <div key={step.title} className="flex items-start gap-3">
              ...
              <div>
                <div className="text-sm font-semibold text-white leading-snug">{step.title}</div>
      ...
      <div className="w-full flex flex-col gap-2.5 mb-4">
        <Link
          href="/dashboard"
          className="w-full flex items-center justify-center py-3.5 bg-green-600 text-white rounded-full text-sm font-semibold hover:bg-green-700 transition-colors"
        >
          Go to Dashboard
        </Link>
        <Link
          href="/billing/plans"
          className="w-full flex items-center justify-center gap-1 py-3.5 bg-transparent text-white border border-zinc-700 rounded-full text-sm font-semibold hover:border-white transition-colors"
        >
