...
const feeItems = [
  ...
  { name: 'IRS Submission', amount: '$500', status: 'Upcoming', statusBg: 'bg-[#F1F5F9]', statusColor: 'text-[#94A3B8]', sub: 'Due on submission', muted: true },
  { name: 'Representation', amount: '$1,000', status: 'Upcoming', statusBg: 'bg-[#F1F5F9]', statusColor: 'text-[#94A3B8]', sub: 'Due on engagement', muted: true },
  { name: 'Post-Resolution', amount: '$500', status: 'Upcoming', statusBg: 'bg-[#F1F5F9]', statusColor: 'text-[#94A3B8]', sub: 'Due on resolution', muted: true },
]
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex items-center justify-between px-5 pt-4 pb-3">
        ...
        <h1 className="text-lg font-bold text-[#0A1628]">Service Agreement</h1>
        <div className="w-9" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-6 space-y-4">
        {/* Title */}
        <div>
          <h2 className="text-xl font-extrabold text-[#0A1628] leading-tight">Your Tax Professional Engagement</h2>
          <p className="text-sm text-[#94A3B8] mt-1">Contract details and payment tracking</p>
        </div>

        {/* Active Contract Card */}
        <div className="bg-white rounded-2xl p-[18px] text-[#0A1628]">
          <div className="flex items-center justify-between mb-3.5">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-full bg-[#F1F5F9] flex items-center justify-center">
                <svg className="h-[18px] w-[18px] text-[#94A3B8]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                </svg>
              </div>
              <div>
                <p className="text-[0.9375rem] font-extrabold">Michael Chen, EA</p>
                <p className="text-[0.6875rem] text-[#94A3B8]">Enrolled Agent</p>
              </div>
            </div>
          ...
          <div className="border-t border-[#F1F5F9] pt-3 space-y-2">
            <div>
              <p className="text-[0.625rem] text-[#64748B] uppercase tracking-wide">Service</p>
              <p className="text-xs font-semibold">OIC Preparation &amp; IRS Representation</p>
            </div>
            <div>
              <p className="text-[0.625rem] text-[#64748B] uppercase tracking-wide">Contract Date</p>
        ...
        {/* Fee Structure Card */}
        <div className="bg-white border border-[#F1F5F9] rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-[#0A1628]">Fee Structure</span>
            </div>
            <span className="text-[0.9375rem] font-black text-[#0A1628]">$3,500</span>
          </div>

          <div className="flex flex-col">
            {feeItems.map((item, i) => (
              <div key={item.name} className={`flex items-center justify-between py-2.5 ${i < feeItems.length - 1 ? 'border-b border-[#F1F5F9]' : ''}`}>
                <div>
                  <p className={`text-sm font-semibold ${item.muted ? 'text-[#94A3B8]' : 'text-[#0A1628]'}`}>{item.name}</p>
                  {item.sub && <p className="text-[0.625rem] text-[#94A3B8] mt-0.5">{item.sub}</p>}
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-sm font-bold ${item.muted ? 'text-[#94A3B8]' : 'text-[#0A1628]'}`}>{item.amount}</span>
          ...
          <div className="mt-3.5">
            <div className="flex justify-between mb-1.5">
              <span className="text-[0.6875rem] font-bold text-[#94A3B8]">$750 / $3,500 paid</span>
              <span className="text-[0.6875rem] font-extrabold text-[#0A1628]">21%</span>
            </div>
        ...
        <div className="flex items-center gap-2.5 p-3 bg-[#2563EB]/10 rounded-xl">
          ...
          <p className="text-sm text-[#0A1628] font-semibold">Retainer balance: $250 remaining</p>
        </div>
        ...
        <div className="bg-white border border-[#F1F5F9] rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-bold text-[#0A1628]">Contract Documents</span>
          </div>

          {documents.map((doc, i) => (
            <div key={doc.name} className={`flex items-center justify-between py-2.5 ${i < documents.length - 1 ? 'border-b border-[#F1F5F9]' : ''}`}>
        ...
        <div className="flex gap-2.5">
          <Link href="#" className="flex-1 text-center py-3 bg-white border border-[#F1F5F9] rounded-xl text-sm font-semibold text-[#0A1628]">
            <span className="mr-1">&#128196;</span> View Full Contract
          </Link>
          <Link href="/expert" className="flex-1 text-center py-3 bg-white border border-[#F1F5F9] rounded-xl text-sm font-semibold text-[#0A1628]">
