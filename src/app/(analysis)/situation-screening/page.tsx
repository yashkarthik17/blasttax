    ...
    <div className="mx-auto max-w-2xl space-y-5">
      ...
      <div>
        <div className="h-1.5 w-full overflow-hidden rounded-full bg-zinc-800">
          <div className="h-full rounded-full bg-blue-500 transition-all duration-500" style={{ width: '45%' }} />
        </div>
        <div className="mt-1.5 flex items-center justify-between">
          <span className="text-xs font-semibold text-zinc-500">Step 3 of 6</span>
          <span className="text-xs font-semibold text-blue-400">Special Circumstances</span>
        </div>
      </div>
          ...
          return (
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-xl border p-4 text-left transition-all ${
                isSelected
                  ? 'border-white/20 bg-blue-500/10 ring-1 ring-blue-500/20'
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <span className="block text-sm font-bold text-white">{spec.label}</span>
                <span className="block text-xs text-zinc-500">{spec.description}</span>
              </div>
              <div
                className={`flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full transition-all ${
                  isSelected ? 'scale-100 bg-white opacity-100' : 'scale-75 bg-zinc-700 opacity-0'
                }`}
              >
      ...
      <div className="text-center">
        <button
          type="button"
          onClick={selectNone}
          className={`text-sm font-semibold transition-colors ${noneSelected ? 'text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
        >
          None of these apply
        </button>
      </div>
      ...
      <div className="flex gap-3 pt-4">
        <button
          onClick={() => router.back()}
          className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl border border-zinc-700 bg-zinc-800/50 text-zinc-400 transition-all hover:bg-zinc-700 hover:text-white"
        >
        ...
        <button
          ...
          className="flex flex-1 items-center justify-center gap-2 rounded-full bg-blue-600 py-3.5 text-sm font-bold text-white transition-all hover:bg-blue-500 active:scale-[0.98] disabled:pointer-events-none disabled:opacity-40"
        >
          Continue
