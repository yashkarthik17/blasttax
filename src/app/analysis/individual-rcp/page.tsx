...
function AssetRow({ label, detail, detail2, amount, qsv }: { label: string; detail: string; detail2?: string; amount: string; qsv?: string }) {
  return (
    <div className="flex items-start justify-between border-b border-[#F1F5F9] py-2.5 last:border-0">
      <div>
        <div className="text-sm font-semibold text-[#0A1628]">{label}</div>
        <div className="text-xs text-[#94A3B8]">{detail}</div>
        {detail2 && <div className="text-xs text-[#94A3B8]">{detail2}</div>}
      </div>
...
export default function IndividualRCPPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      {/* Heading */}
      <div>
        <h1 className="text-2xl font-extrabold text-[#0A1628]">Your Reasonable Collection Potential</h1>
        <p className="mt-1 text-sm text-[#64748B]">How the IRS calculates what they can collect from you</p>
      </div>
      ...
      <div className="rounded-2xl border border-[#F1F5F9] bg-white p-4">
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-bold text-[#0A1628]">Section 1: Net Realizable Equity (NRE)</div>
            <div className="text-xs text-[#94A3B8]">Per-asset QSV breakdown</div>
          </div>
        </div>

        <h4 className="mb-1 text-[10px] font-bold uppercase tracking-wider text-[#94A3B8]">Bank Accounts</h4>
        <AssetRow label="Cash & Checking" detail="No discount -- full balance" amount="$4,200" qsv="$4,200" />

        <h4 className="mb-1 mt-3 text-[10px] font-bold uppercase tracking-wider text-[#94A3B8]">Real Estate</h4>
        <AssetRow label="Primary Residence" detail="FMV $320K x 80% = $256K" detail2="- $285K mtg - $20K HELOC" amount="-$49,000" qsv="$0" />

        <h4 className="mb-1 mt-3 text-[10px] font-bold uppercase tracking-wider text-[#94A3B8]">Vehicles</h4>
        <AssetRow label="2020 Honda Civic" detail="FMV $18.5K x 80% = $14.8K" detail2="- $13.3K loan" amount="$1,500" qsv="$1,500" />

        <h4 className="mb-1 mt-3 text-[10px] font-bold uppercase tracking-wider text-[#94A3B8]">Retirement Accounts</h4>
        <AssetRow label="401(k) -- Fidelity" detail="Balance $42K - 10% penalty - 25% tax" amount="$28,350" qsv="$28,350" />

        <h4 className="mb-1 mt-3 text-[10px] font-bold uppercase tracking-wider text-[#94A3B8]">Life Insurance</h4>
        <AssetRow label="Whole Life -- Net CSV" detail="CSV $4,500 - $1,300 loans" amount="$3,200" qsv="$3,200" />

        <h4 className="mb-1 mt-3 text-[10px] font-bold uppercase tracking-wider text-[#94A3B8]">Other Assets</h4>
        <div className="flex items-center justify-between py-2.5">
          <span className="text-sm text-[#94A3B8]">None reported</span>
          <span className="text-sm font-bold text-[#0A1628]">$0</span>
        </div>

        {/* NRE Total */}
        <div className="mt-3 flex items-center justify-between rounded-xl bg-[#F8FAFC] px-4 py-3">
          <span className="text-[10px] font-semibold uppercase tracking-wider text-[#64748B]">Total NRE</span>
          <span className="text-lg font-black text-[#0A1628]">{fmt(nre)}</span>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[#F1F5F9] bg-white p-4">
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-bold text-[#0A1628]">Section 2: Future Income</div>
            <div className="text-xs text-[#94A3B8]">MDI x payment period multiplier</div>
          </div>
        </div>

        <div className="mb-3 flex items-center justify-between rounded-lg bg-[#F8FAFC] px-3 py-2.5">
          <span className="text-sm text-[#64748B]">Monthly Disposable Income (MDI)</span>
          <span className="text-sm font-bold text-[#0A1628]">$511/mo</span>
        </div>
        ...
        <div className="mb-3 grid grid-cols-2 gap-2">
          {(['lump', 'periodic'] as const).map((type) => (
            ...
            >
              <div className={`mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full border-2 transition-all ${paymentType === type ? 'border-blue-500 bg-[#2563EB]' : 'border-[#E2E8F0]'}`}>
                {paymentType === type && <div className="h-2 w-2 rounded-full bg-white" />}
              </div>
              <div>
                <div className="text-sm font-bold text-[#0A1628]">{type === 'lump' ? 'Lump Sum' : 'Periodic'}</div>
                <div className="text-xs text-[#94A3B8]">MDI x {type === 'lump' ? '12' : '24'} months</div>
              </div>
            </button>
          ))}
        </div>

        <div className="mb-2 flex items-center justify-between rounded-lg bg-[#F8FAFC] px-3 py-2.5">
          <span className="text-sm text-[#64748B]">Multiplier</span>
          <span className="text-sm font-bold text-[#0A1628]">x {months} months</span>
        </div>
        <div className="flex items-center justify-between rounded-lg bg-[#2563EB]/10 px-3 py-3">
          <span className="text-sm font-bold text-[#0A1628]">Future Income</span>
      ...
      <div className="rounded-2xl bg-gradient-to-br from-zinc-900 to-zinc-800 p-5">
        <h4 className="mb-4 text-[10px] font-semibold uppercase tracking-wider text-[#64748B]">Section 3: Total RCP</h4>
        <div className="flex items-center justify-between border-b border-[#F1F5F9] py-2">
          <span className="text-sm text-[#64748B]">Net Realizable Equity (NRE)</span>
          <span className="text-sm font-bold text-[#0A1628]">{fmt(nre)}</span>
        </div>
        <div className="flex items-center justify-between border-b border-[#F1F5F9] py-2">
          <span className="text-sm text-[#64748B]">Future Income</span>
          <span className="text-sm font-bold text-[#0A1628]">{fmt(futureIncome)}</span>
        </div>
        <div className="mt-2 flex items-center justify-between py-3">
          <span className="text-lg font-extrabold text-[#0A1628]">Total RCP</span>
      ...
      <div className="flex items-start gap-3 rounded-xl bg-[#00A651]/10 px-4 py-3 ring-1 ring-emerald-500/20">
        ...
        <div>
          <div className="text-sm font-bold text-[#0A1628]">This is your minimum OIC offer</div>
      ...
      <div className="rounded-2xl border border-[#F1F5F9] bg-white overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-bold text-[#0A1628]">How to lower your RCP</span>
          </div>
          <svg className={`h-3 w-3 text-[#94A3B8] transition-transform ${tipsOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            ...
            {[
              ...
              <div key={tip.num} className="flex items-start gap-3">
                ...
                <p className="text-xs text-[#64748B] leading-relaxed">
                  <strong className="text-[#0A1628]">{tip.title}</strong> {tip.text}
                </p>
              </div>
            ))}
          </div>
      ...
      <button
        onClick={() => router.back()}
        className="flex w-full items-center justify-center gap-2 rounded-full bg-[#F8FAFC] px-6 py-4 text-base font-bold text-[#0A1628] transition-all hover:bg-[#F1F5F9] active:scale-[0.98]"
      >
