            ...
            {STEPS.map((step, i) => {
              ...
              return (
                <div key={step.label} className="flex items-center gap-3">
                  ...
                  <span className={`text-sm ${status === 'done' ? 'text-zinc-400' : status === 'active' ? 'font-medium text-white' : 'text-zinc-600'}`}>
                    {step.label}
                  </span>
                </div>
              )
        ...
        {error && (
          <div className="space-y-4">
            ...
            <button onClick={handleRetry} disabled={retrying}
              className="w-full rounded-xl bg-emerald-600 py-4 text-base font-semibold text-white transition-colors hover:bg-emerald-500 disabled:bg-zinc-800 disabled:text-zinc-600">
              {retrying ? 'Retrying...' : 'Retry Analysis'}
            </button>
            <button onClick={() => router.push('/analysis/verification')}
              className="w-full rounded-xl border border-zinc-700 py-3 text-sm font-medium text-zinc-400 hover:border-zinc-600 hover:text-white">
