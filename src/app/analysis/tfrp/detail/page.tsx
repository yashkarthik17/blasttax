  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAFF' }}>
      <div style={{ maxWidth: '28rem', margin: '0 auto' }}>
        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'white', padding: '12px 20px', borderBottom: '1px solid #F0F0F5' }}>
          <button onClick={() => router.back()} style={{ width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '12px', background: '#FAFAFF', border: '1px solid #F0F0F5', cursor: 'pointer' }}>
            <i className="fa-solid fa-arrow-left" style={{ color: '#5C5C7A' }} />
          </button>
          <span style={{ fontSize: '15px', fontWeight: 700, color: '#1A1A2E' }}>Trust Fund Recovery Penalty</span>
        ...
        <div style={{ padding: '20px', paddingBottom: '120px' }}>
          ...
          <div style={{ background: '#FFF0F1', border: '1.5px solid #FECACA', borderRadius: '16px', padding: '16px', marginBottom: '18px' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
              ...
              <p style={{ fontSize: '0.8125rem', color: '#1A1A2E', fontWeight: 600, lineHeight: 1.5 }}>TFRP makes you <span style={{ color: '#E63946', fontWeight: 800 }}>PERSONALLY</span> liable for the trust fund portion of unpaid payroll taxes</p>
            </div>
          </div>

          {/* What Is Trust Fund */}
          <div style={{ background: 'white', border: '1.5px solid #F0F0F5', borderRadius: '16px', padding: '16px', marginBottom: '14px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
              <i className="fa-solid fa-vault" style={{ color: '#1A1A2E', fontSize: '14px' }} />
              <span style={{ fontSize: '0.8125rem', fontWeight: 700, color: '#1A1A2E' }}>What Is Trust Fund?</span>
            </div>
            <p style={{ fontSize: '0.75rem', color: '#5C5C7A', lineHeight: 1.6 }}>The employee&apos;s share of Social Security, Medicare, and withheld income tax &mdash; money held <em>in trust</em> for the government.</p>
          </div>
          ...
          <div style={{ background: '#FFFBFB', border: '1.5px solid rgba(230,57,70,0.2)', borderRadius: '16px', padding: '16px', marginBottom: '18px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <p style={{ fontSize: '0.75rem', color: '#1A1A2E', fontWeight: 500 }}>This TC on your personal transcript means TFRP has been assessed</p>
            </div>
          </div>
          ...
          <div style={{ background: 'white', border: '1.5px solid #F0F0F5', borderRadius: '16px', padding: '16px', marginBottom: '14px' }}>
            <div style={{ marginBottom: '12px' }}>
              <p style={{ fontSize: '0.75rem', fontWeight: 700, color: '#1A1A2E', marginBottom: '6px' }}>Who qualifies:</p>
              <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '6px' }}>
                {[
                  ...
                  <span key={p.label} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '6px 12px', borderRadius: '10px', fontSize: '0.75rem', fontWeight: 600, background: '#FAFAFF', border: '1px solid #F0F0F5' }}>
                    <i className={`fa-solid ${p.icon}`} style={{ fontSize: '11px', color: '#1A1A2E' }} /> {p.label}
                  </span>
                ))}
              </div>
            </div>

            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', padding: '10px', background: '#FAFAFF', borderRadius: '10px', marginBottom: '10px' }}>
              <i className="fa-solid fa-clipboard-question" style={{ fontSize: '13px', color: '#7C3AED', marginTop: '2px' }} />
              <div>
                <p style={{ fontSize: '0.75rem', fontWeight: 700, color: '#1A1A2E', marginBottom: '2px' }}>Form 4180 Interview</p>
          ...
          <div style={{ background: 'white', border: '1.5px solid rgba(0,61,165,0.15)', borderRadius: '16px', padding: '16px', marginBottom: '18px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '12px', borderTop: '2px solid #1A1A2E', marginTop: '4px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '10px', marginBottom: '18px' }}>
            {RESOLUTION_OPTIONS.map((opt) => (
              <div key={opt.num} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '14px', background: 'white', border: '1.5px solid #F0F0F5', borderRadius: '14px', cursor: 'pointer', transition: 'all 0.5s ease' }}>
                <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: '#EBF0FF', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '0.75rem', fontWeight: 700, color: '#1A1A2E' }}>{opt.num}</div>
                <div>
                  <p style={{ fontSize: '0.8125rem', fontWeight: 700, color: '#1A1A2E', marginBottom: '2px' }}>{opt.title}</p>
                  <p style={{ fontSize: '0.6875rem', color: '#5C5C7A', lineHeight: 1.5 }}>{opt.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ position: 'sticky' as const, bottom: 0, padding: '16px 20px', paddingBottom: '28px', background: '#FFFFFF', zIndex: 10 }}>
          ...
          <p style={{ textAlign: 'center' as const, marginTop: '10px' }}>
            <a href="#" style={{ fontSize: '0.8125rem', color: '#1A1A2E', fontWeight: 600, textDecoration: 'none' }}>Talk to Expert <i className="fa-solid fa-arrow-right" style={{ fontSize: '10px' }} /></a>
          </p>
        </div>
      </div>
    </div>
